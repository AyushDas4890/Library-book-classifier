...
export default function SearchPanel({ engine, onSimilar }: { engine: RetrievalEngine | null; onSimilar: (h: Hit) => void }) {
  ...
  return (
    <div>
      ...
      {hits && noTokens && (
        <p style={{ color: "var(--faint)", marginTop: 18 }}>No indexed terms in that query — try one of the examples above.</p>
      )}
      ...
      {hits && !noTokens && (
        <div className="results">
          <AnimatePresence mode="popLayout">
            {hits.map((h, i) => (
              <motion.div className="card row" key={h.id}
                ...
                <div className="right">
                  ...
                  <div className="scorebar"><i style={{ width: `${Math.max(6, Math.min(100, h.score * 100))}%` }} /></div>
